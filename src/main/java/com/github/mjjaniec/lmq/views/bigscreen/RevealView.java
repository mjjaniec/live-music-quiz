...
public class RevealView extends VerticalLayout implements BigScreenRoute {
    ...
    public RevealView(GameService gameService, TestDataProvider testDataProvider) {
        ...
        gameService.pieceStage().or(testDataProvider::piece).ifPresent(piece -> {
            String artist = piece.piece.artist()
                    + (Optional.ofNullable(piece.piece.artistAlternative())
                            .map(a -> " / " + a)
                            .orElse(""));
            String title = piece.piece.title()
                    + (Optional.ofNullable(piece.piece.titleAlternative())
                            .map(a -> " / " + a)
                            .orElse(""));
