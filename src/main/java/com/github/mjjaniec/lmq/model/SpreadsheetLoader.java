...
public class SpreadsheetLoader {
    ...
    private final CsvMapper mapper =
            CsvMapper.builder().enable(CsvParser.Feature.WRAP_AS_ARRAY).build();
    private @Value(
            "https://docs.google.com/spreadsheets/d/${application.csm-spreadsheet-id}/gviz/tq?tqx=out:csv&sheet=")
    String BaseUrl;

    @SneakyThrows
    public PlayOffs loadPlayOffs() {
        try (MappingIterator<String[]> readValues = mapper.readerFor(String[].class)
                .with(csvSchema)
                .readValues(new URI(BaseUrl + PlayOff).toURL().openStream())) {
            return new PlayOffs(readValues.readAll().stream()
                    .map(array ->
                            new PlayOffs.PlayOff(array[1], Integer.parseInt(array[0]), Integer.parseInt(array[2])))
                    .toList());
        }
    }
    ...
    public MainSet loadMainSet() {
        try (MappingIterator<String[]> readValues = mapper.readerFor(String[].class)
                .with(csvSchema)
                .readValues(new URI(BaseUrl + SetList).toURL().openStream())) {
    ...
    private List<String> loadStrings(String tab) {
        try (MappingIterator<String[]> readValues = mapper.readerFor(String[].class)
                .with(csvSchema)
                .readValues(new URI(BaseUrl + tab).toURL().openStream())) {
