...
public class ResultsTable extends Grid<Results.Row> {
    ...
    public ResultsTable(Results results, int showFrom, boolean showAwards) {
        ...
        addColumn("Nagroda", 0, row -> {
            H3 res = new H3(row.award()
                    .filter(_ -> showPlayoff && showAwards)
                    .map(a -> a.symbol)
                    .orElse(""));
            res.getStyle().setLineHeight("2.2rem");
        ...
        addColumn(
                ...
                row -> testId(
                        text(
                                results.rounds() == results.currentRound()
                                        ? row.playOff() + " / " + results.targetPlayOff()
                                        : ""),
                        "big-screen/results/playoff-" + row.position()));
        addColumn(
                "Razem",
                1,
                row -> testId(boldText(String.valueOf(row.total())), "big-screen/results/total-" + row.position()));

        setPartNameGenerator(row -> Stream.concat(
    ...
    private void addColumn(String caption, int flexGrow, Function<Results.Row, Component> renderer) {
        addColumn(new ComponentRenderer<>((SerializableFunction<Results.Row, Component>) renderer::apply))
                .setHeader(caption)
